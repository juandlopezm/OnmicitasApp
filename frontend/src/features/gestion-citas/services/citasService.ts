import type { Cita, Especialidad, HorarioDisponible, Medico, NuevaCita, Sede } from '../../../types';
...
export async function getMedicosByEspecialidad(especialidadId: string): Promise<Medico[]> {
  const data = await api.get<Array<{ id: number; nombres: string; apellidos: string; especialidad_id: number }>>
...
export async function getHorariosDisponibles(
  ...
  // catalog-service aplica el filtro de 24 h de anticipación automáticamente
  const data = await api.get<Array<{
    id: number; medico_id: number; sede_id: number; fecha: string; hora_inicio: string; estado: string;
  }>>(`/api/horarios/disponibles?medico_id=${medicoId}&fecha=${fecha}`);
...
function mapCita(c: BackendCita): Cita {
  return {
    ...
    beneficiarioId: c.beneficiario_id != null ? String(c.beneficiario_id) : undefined,
...
export async function crearCita(
  _usuarioId: string,
  nuevaCita: NuevaCita & { beneficiarioId?: string }
): Promise<Cita> {
  const data = await api.post<BackendCita>('/api/citas', {
    ...
    medico_nombre: nuevaCita.medicoNombre ?? '',
    especialidad_nombre: nuevaCita.especialidadNombre ?? '',
    sede_nombre: nuevaCita.sedeNombre ?? '',
...
export async function crearCitaAdmin(datos: {
  ...
  try {
    const data = await api.post<BackendCita>('/api/admin/citas', {
      ...
      especialidad_nombre: datos.especialidadNombre,
      sede_nombre:        datos.sedeNombre,
      notas:              datos.notas,
    });
...
export async function reagendarCita(
  ...
  try {
    await api.put(`/api/citas/${citaId}/reagendar`, {
      horario_id: Number(nuevoHorarioId),
