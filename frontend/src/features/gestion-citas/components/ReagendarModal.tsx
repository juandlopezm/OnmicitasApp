...
import { obtenerFechaEnDias, formatearHora } from '../../../utils/dateUtils';
...
export function ReagendarModal({ cita, onClose, onSuccess }: ReagendarModalProps) {
  ...
  const maxFecha = obtenerFechaEnDias(7);
  ...
  return (
    <Dialog
      ...
      description={exito ? undefined : cita ? `${cita.especialidadNombre} — ${cita.medicoNombre}` : undefined}
    >
      {exito ? (
        <div className="flex flex-col items-center gap-4 py-4 text-center">
          ...
          <div>
            <p className="font-semibold text-base">¡Cita reagendada correctamente!</p>
            <p className="text-sm text-muted-foreground mt-1">
              Tu cita ha sido actualizada con la nueva fecha y hora.
            </p>
          </div>
